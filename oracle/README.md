...
Установите скрипт `SysV` [`rc.status`](rc.status) из данного репозитория прямо
в корень папки `/etc`. Вы можете воспользоваться командой `scp` через
[`CygWin`](../README.md#cygwin), если работаете на `Windows`-е:
