...
Для перехода из второго режима в первый, используется клавиша `<ESC>`. Для
обратного перехода команды ввода текста. Вообще всю архитектуру
...
*Субъектные* команды же представлены следующим шаблоном:

`сказуемое обстоятельство подлежащее`
...
*Подлежащеее*, это **объект** воздействия, к примеру `w` - слово. Пример:

> `d20w`: *удалить 20 слов*

В осутствии *сказуемого* предполагается действие *навигации* - перемещения.
Пример:
...
Так же у *сказуемого* есть условное *наклонение* глагола. Команда поданная с
малой буквой обычно производит небольшие изменения, а с большой - большие.
Пример:
...
Сначала пройдите святой `vimtutor`. После прохождения `vimtutor` запустите сам
`Vim` и ознакомтесь с системой помощи. Помощь по конкретному разделу вызывается
командой `:h[elp] <раздел>`. Когда открываете помощь, увидите как бы ссылки,
это оглавление и `slug`-и глав. Эти `slug`-и используются так же, как опция при
вызове `:h`. Вы можете пройти по ссылке встав на неё, и нажав на `Ctrl + ]`.
Папримеры вызова справок:
